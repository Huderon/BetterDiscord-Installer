...
func (discord *DiscordInstall) uninject() error {
	...
	if !utils.Exists(originalAsar) && !utils.Exists(preservedAsar) {
		log.Printf("❌ Unable to uninject %s: missing both app.asar and betterdiscord.app.asar\n", discord.Channel.Name())
		log.Printf("   Your %s install appears corrupted! Reinstall Discord, then run the installer.\n", discord.Channel.Name())
