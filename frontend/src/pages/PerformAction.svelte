<script>
    ...
    import {log, reset, succeed, fail} from "../actions/log";
    ...
    if (currentAction === "install") {
        ...
        install(Object.keys(installPaths), Object.values(installPaths)).then(() => {
            nextPage.set(null);
        ...
        repair(Object.keys(installPaths), Object.values(installPaths)).then(() => {
            nextPage.set(null);
            canGoForward.set(true);
            canGoBack.set(true);
        });
        ...
        uninstall(Object.keys(installPaths), Object.values(installPaths)).then(() => {
            nextPage.set(null);
            canGoForward.set(true);
            canGoBack.set(true);
        });
...
<section class="page" in:page out:page="{{out: true}}">
    ...
    <ProgressBar value={$progress} class={$status} indeterminate={$status.value === ""} />
</section>